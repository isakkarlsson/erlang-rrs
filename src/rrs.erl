...
main(Args) ->
    Options = rr:parse("rrs", Args, ?CMD_SPEC),
    ...
    Info = lists:foldl(
             fun (File, Acc) ->
                     ...
                     try
                         ...
                         #rr_exset{features=Features, 
                                   examples=Examples, 
                                   exconf=ExConf} = rr_example:load(Csv, erlang:system_info(schedulers)),
                         ...
                         X:_ ->
                             rr_log:info("failed to parse ~s (~p)", [File, X]),
                             Acc
                     end                                      
             end, [], Filenames),
